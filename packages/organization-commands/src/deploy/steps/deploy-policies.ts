...
export const deployPolicies: DeployOrganizationStep<BasicConfigDeploymentResultHolder> =
  async (state) => {
    const { transitions, io, ctx, policiesPlan, organizationState } = state

    const { hasChanges, aiServicesOptOut, backup, serviceControl, tag } =
      policiesPlan
    ...
    const addedPolicies: ReadonlyArray<PolicyDeploymentResult> =
      await Promise.all(
        policiesToAdd.map((p) =>
          client
            .createPolicy({
    ...
    const updatedPolicies: ReadonlyArray<PolicyDeploymentResult> =
      await Promise.all(
