...
describe("Command paths", () => {
  test("Deploying '/dev/app/logs.yml' does not require providing values for variables outside the stack", async () => {
  ...
  test("Deploying '/dev/app/sg.yml' causes stack '/dev/vpc.yml' to be deployed as well", async () => {
    const { options, variables, watch } = await createOptions([])
    const output = await deployStacksCommand(
      {
        options,
