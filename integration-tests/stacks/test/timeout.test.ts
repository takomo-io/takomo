...
describe("Timeout", () => {
  test("Deploy should fail due timeout", async () => {
  ...
  test("Deploying should fail due timeout", async () => {
    const { options, variables, watch } = await createOptions([
      "create_wait_condition=true",
  ...
  test("Deploying stack again should succeed when the failing resources are not created", async () => {
    const { options, variables, watch } = await createOptions([
      "create_wait_condition=false",
      "create_second_topic=true",
    ])
